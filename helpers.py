...
TARGET_REPO_DIR = os.path.join(WORK_DIR, "ofgo", "repositories")
...
def validate_repo_url(url: str) -> str:
    ...
    try:
        ...
        _log(f"ERROR! {url} does not exist: \n{e}")
        sys.exit(1)
...
def validate_model(model, temperature):
    ...
    try:
        ...
        _log('''Program execution failed: Missing OpenAI API Key.
Make sure to export your API Key via the following command:
   export OPENAI_API_KEY={your_api_key}''')
    ...
    try:
        ...
        _log(f'''Failed to generate test response. OpenAI API response:
{e}''')
        sys.exit(1)
    return 

def check_project_compilation(project_name):
    ...
    _log(f"Testing with OSS-Fuzz to ensure compilation:")
    ...
    if not os.path.exists(fuzz_dir):
        _log(f"Could not locate fuzzing harnesses at {fuzz_dir}")
        sys.exit(1)
    ...
    for root, dirs, files in os.walk(fuzz_dir):
        for file in files:
            if (file.startswith("fuzz_") or file.endswith("Fuzzer") or re.match(r'^fuzz-harness-\d+_\d+\$', file)) and '.' not in file:
                fuzzers.append(file)
    if not fuzzers:
        _log(f"Unable to locate any fuzzing harnesses in {fuzz_dir}")
        sys.exit(1)
    for fuzzer in fuzzers:
        res = subprocess.run(["python3", INFRA_HELPER, "run_fuzzer", project_name, fuzzer, "--", "-max_total_time=30", "-runs=100"], capture_output=True)
        if res.returncode != 0:
            _log(f'\n{res.stderr.decode()}')
            _log(f"Failed to run {fuzzer}.")
            sys.exit(1)
        else:
            _log(f"Successfully ran {fuzzer}.")
    _log(f"All fuzzers ran successfully.")